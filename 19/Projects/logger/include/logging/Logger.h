...
namespace logging {
    ...
    public:
        ...
        // 提交严重错误信息（log的包装）
        Logger& critical(const std::string& message, std::source_location sourceLocation = std::source_location::current()) {
            return log<Level::Critical>(message, sourceLocation);
        }

        // 提交一般错误信息（log的包装）
        Logger& error(const std::string& message, std::source_location sourceLocation = std::source_location::current()) {
            return log<Level::Error>(message, sourceLocation);
        }

        // 提交警告信息（log的包装）
        Logger& warning(const std::string& message, std::source_location sourceLocation = std::source_location::current()) {
            return log<Level::Warning>(message, sourceLocation);
        }

        // 提交普通信息（log的包装）
        Logger& info(const std::string& message, std::source_location sourceLocation = std::source_location::current()) {
            return log<Level::Info>(message, sourceLocation);
        }
        ...
        // 提交程序跟踪信息（log的包装）
        Logger& trace(const std::string& message, std::source_location sourceLocation = std::source_location::current()) {
            return log<Level::Trace>(message, sourceLocation);
        }
